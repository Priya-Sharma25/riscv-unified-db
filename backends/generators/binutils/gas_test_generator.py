...
def extract_instruction_constraints(name: str, data: dict) -> dict:
    ...
    for var in variables:
        ...
        if var_name in ["xd", "xs1", "xs2", "xs3", "rd", "rs1", "rs2", "rs3"]:
            ...
            var_name in ["imm", "simm"]
            or var_name.startswith("zimm")
            ...
            if is_signed:
                ...
                if width > 0:
                    ...
                    max_val, min_val = (
                        DEFAULT_12BIT_UNSIGNED_IMM_MAX,
                        DEFAULT_12BIT_UNSIGNED_IMM_MIN,
                    )
...
class AssemblyExampleGenerator:
    ...
    def __init__(
        self,
        csr_dir: str = "../../../spec/std/isa/csr/",
        inst_dir: str = "../../../spec/std/isa/inst/",
    ):
    ...
    def _load_csr_examples(self):
        ...
        try:
            ...
            self.csr_examples = list(
                {name.lower().replace(".rv32", "") for name in csr_dict.values()}
            )[:MAX_CSR_EXAMPLES]
        except Exception as e:
            logging.warning(
                f"Failed to load CSRs from {self.csr_dir}: {e}. Using fallback CSR list."
            )
    ...
    def _load_all_instruction_data(self) -> tuple[dict[str, dict], dict[str, dict]]:
        ...
        logging.debug(
            f"Single-pass loaded {len(instruction_data)} instructions, {len(instruction_constraints)} constraints"
        )
    ...
    def _get_operand_replacements(
        ...
        if "imm_range" in constraints:
            ...
            safe_imm = self._get_safe_immediate_from_constraints(
                min_val, max_val, imm_multiple, imm_not_zero, i
            )
    ...
    def _get_safe_immediate_from_constraints(
        self, min_val: int, max_val: int, multiple: int, not_zero: bool, variant: int
    ) -> int:
        ...
        for candidate in candidates:
            if (
                min_val <= candidate <= max_val
                and candidate % multiple == 0
                and (not not_zero or candidate != 0)
            ):
                return candidate
        ...
        for i in range(min(3, len(reg_set) - 1)):
            ...
            for operand in operands:
                ...
                if operand_type == "rounding_mode" or operand_raw == "rm":
                    replacements["rm"] = self.rounding_mode_examples[
                        i % len(self.rounding_mode_examples)
                    ]
                elif operand_type == "fence_ordering" or operand_raw in [
                    "pred",
                    "succ",
                ]:
                    if operand_raw == "pred":
        ...
        for part in parts:
            ...
            if "(" in part and ")" in part:
                ...
                if match:
                    ...
                    if base_reg == "sp":
                        operand_info.update({"type": "memory_sp", "offset": "4"})
                    else:
                        operand_info.update(
                            {
                                "type": "memory",
    ...
    def _get_safe_immediate(self, name: str, constraints: dict) -> int:
        ...
        imm_range = constraints.get(
            "imm_range", (DEFAULT_12BIT_SIGNED_IMM_MIN, DEFAULT_12BIT_SIGNED_IMM_MAX)
        )
        imm_multiple = constraints.get("imm_multiple", 1)
        imm_not_zero = constraints.get("imm_not_zero", False)
        ...
        for candidate in candidates:
            if (
                min_val <= candidate <= max_val
                and candidate % imm_multiple == 0
                and (not imm_not_zero or candidate != 0)
            ):
                return candidate
...
class GasTestGenerator:
    ...
    def _create_disasm_pattern(
        self, addr: int, name: str, example: str, no_aliases: bool = False
    ) -> str:
